...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `QUE PENA QUE ELE SAIU TOMARA QUE ELE NÃO VOLTE MAIS @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Espera Ae rapidão ja ta saindo do forno',
				success: 'Pronto tá ae Nem demorou',
				error: {
					stick: 'Desculpa Por Não fazer ',
					Iv: 'Verifica esse link ae'
				},
				only: {
					group: 'Esse comando só pode ser usado em grupo',
					ownerG: ' Você Não é ademir',
					ownerB: '❌ so o dono do bot pode usar esse comando❌',
					admin: 'Você não é o dono❌',
					Badmin: 'Da adm pro bot primeiro'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["557488593773@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'nsfwloli':
					...
					break*/
				case 'hilih':
					if (args.length < 1) return reply('Cade o texto menó')
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					reply(anu.result)
					break
		
				case 'yt2mp3':
					if (args.length < 1) return reply('e o link amigão')
				...
				case 'ytsearch':
					if (args.length < 1) return reply('ta procurando oq ')
				...
				case 'stalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Cade seu nome de usuário', text, {quoted: mek})
						...
						console.log(`Error :`, color(e,'red'))
						 reply ('Nunca vi esse usuário na minha vida, então usuário inválido'
					}
					break
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Cade o link menó')
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					if (!isOwner) return reply('Quem é a pessoa que está usando este comando') 
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('apagou tudo que pena')
					break
				
				case 'bc':
					if (!isOwner) return reply'quem é está pessoa') 
					...
					if (mentioned.length > 1) {
						teks = 'Parabéns O diggo te deu adm\n'
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'diggo Não gostou da sua atitude então sem adm pra você\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Coloca O número direto ae')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('o número Que você quer adicionar ta privado que pena' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Quer expulsar quem?')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Quem mandou set chato:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`quem mandou ser chato : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
...
//AQUI NAO PRECISA MUDAR                   
				...
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ MARCA UMA FIGURINHA PRIMEIRO❌')
					reply(mess.wait)
...
//NAO FUNCIONA MAIS	
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply(' ATIVOU COM SUCESSO AS BOAS VINDAS NO GRUPO')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
					}
                                      break
